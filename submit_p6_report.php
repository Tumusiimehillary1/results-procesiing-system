...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        $truncateSql = "TRUNCATE TABLE p_6_report";
        if (!$conn->query($truncateSql)) {
            die("Error truncating table: " . $conn->error);
        }

        // Prepare statement for inserting data into p_1_report table
        $stmt = $conn->prepare("INSERT INTO p_6_report (student_name, math, english, sst, science, total, average, position) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        if (!$stmt) {
        ...
        echo "Marks list submitted successfully to p_6_report.";
